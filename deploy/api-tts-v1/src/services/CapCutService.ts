...
import capCutTelegramLoginService from '@/services/CapCutTelegramLoginService';
...
class CapCutService {
  ...
  private activateFallbackRegion(error: unknown, context: string) {
    ...
    this.activeRegionProfile = 'fallback';
    this.voices = null;
    this.voicesLoadedAt = 0;

    logger.warn(
      'CapCut primary region profile failed. Switching to fallback region profile',
      {
        context,
        error,
    ...
    this.sessionPromise = (async () => {
      if (!force && this.session) {
        try {
          ...
          if (shouldRetryWorkspaceList(error) && this.session.workspaceId) {
            logger.info(
              'CapCut session validation hit a transient workspace error. Reusing the cached session',
              { error }
            );
            this.session.verifiedAt = Date.now();
            await this.persistSession();
      ...
      if (!force && this.hasAuthenticatedCookies()) {
        try {
          return await this.bootstrapSessionFromCookies();
        } catch (error) {
          if (shouldRetryWorkspaceList(error)) {
            throw error;
          }

          logger.info(
            'CapCut cookie bootstrap failed. Falling back to a fresh login',
            { error }
          );
        }
      }
  ...
  private async login(): Promise<CapCutSessionState> {
    ...
    if (!hasPasswordLoginCredentials()) {
      if (!capCutTelegramLoginService.isConfigured()) {
        getPasswordLoginCredentials();
      }

      logger.info('CapCut password login is not configured. Using Telegram QR login');
      await capCutTelegramLoginService.login();
      await this.restorePersistedSession();
      return this.bootstrapSessionFromCookies();
    }
    ...
    for (const loginHost of loginHosts) {
      try {
        ...
        return await this.establishSession(loginHost, loginData);
      } catch (error) {
        lastError = error;
   ...
   */
  private async bootstrapSessionFromCookies() {
    logger.info('Attempting CapCut session bootstrap from cached cookies');
    return this.establishSession(this.session?.loginHost ?? null, null);
  }

  private async establishSession(
    loginHost: string | null,
    loginData: LoginResponse | null
  ): Promise<CapCutSessionState> {
    const accountInfo = await this.fetchAccountInfo().catch((error) => {
      logger.info(
        `CapCut account info lookup failed${
          loginHost ? ` after login via ${loginHost}` : ' during cookie bootstrap'
        }`,
        { error }
      );
      return null;
    ...
    const session: CapCutSessionState = {
      ...
      screenName:
        ...
        this.session?.screenName ??
        '',
      workspaceId: workspace.workspace_id,
      loginHost: loginHost ?? this.session?.loginHost ?? env.CAPCUT_LOGIN_HOST,
      verifyFp: this.verifyFp,
      deviceId: this.deviceId,
      loggedInAt: loginData ? Date.now() : this.session?.loggedInAt ?? Date.now(),
      verifiedAt: Date.now(),
    };

    if (!session.workspaceId) {
      throw new Error('CapCut login did not expose a workspace');
    }
    ...
    logger.info('CapCut session established', {
      userId: session.userId || '[unavailable]',
  ...
  private async primeLoginState(loginHost: string) {
    const credentials = getPasswordLoginCredentials();
  ...
  private async fetchPrimaryWorkspace(): Promise<WorkspaceInfo> {
    ...
    for (let attempt = 0; attempt <= retryDelaysMs.length; attempt += 1) {
      try {
        ...
        logger.warn('CapCut workspace list returned a transient error. Retrying', {
          attempt: attempt + 1,
          error,
        });
  ...
  private async createAudioResponseWithRetry(
    ...
    try {
        ...
        try {
          ...
          if (
            this.activateFallbackRegion(
              authError,
              'CapCut session re-authentication'
            )
          ) {
      ...
      if (
        allowRetry &&
        this.activateFallbackRegion(error, 'CapCut TTS request')
      ) {
        return this.createAudioResponseWithRetry(options, false);
      }
  ...
  private async requestSignedEditJson<T>(options: {
    ...
    const tdid =
      options.tdid ??
      this.tdid ??
      this.runtimeEditorBundleConfig.signRecipe?.tdidDefault ??
      '';
...
const getPasswordLoginCredentials = () => {
  if (!env.CAPCUT_EMAIL || !env.CAPCUT_PASSWORD) {
    throw new Error(
      'CapCut password login is not configured. Run the QR login tool first: npm run capcut:qr-login, or configure Telegram QR login.'
    );
  }
...
const hasPasswordLoginCredentials = () =>
  Boolean(env.CAPCUT_EMAIL && env.CAPCUT_PASSWORD);
...
const shouldTryOtherLoginHost = (error: unknown) =>
  ...
  !isPostLoginBootstrapError(error);

const shouldRetryWorkspaceList = (error: unknown) =>
  error instanceof CapCutApiError &&
  error.message.toLowerCase().includes('system busy');

const isPostLoginBootstrapError = (error: unknown) =>
  error instanceof CapCutApiError &&
  /workspace list failed|account info failed/i.test(error.message);
