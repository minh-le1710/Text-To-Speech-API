...
export const createEditApiSignature = (
  ...
  // Some bundles contain unrelated slice(-4) calls before the actual signer.
  // The current edit API signer uses the last 7 path characters.
  const pathTailLength = Math.max(recipe.pathTailLength ?? 7, 7);
